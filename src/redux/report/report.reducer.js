...
const reportReducer = (state = INITIAL_STATE, { type, payload }) => {
    switch (type) {
        ...
        case ReportActionTypes.MANAGE_TEAM_TAB_DISPLAY:
            return { ...state, is_rendered: { team_members: !state.is_rendered.team_members } };

        case ReportActionTypes.MANAGE_DATE_AND_MILESTONE_TAB_DISPLAY:
            return { ...state, is_rendered: { date_and_milestone: !state.is_rendered.date_and_milestone } };

        case ReportActionTypes.MANAGE_SCOPE_TAB_DISPLAY:
            return { ...state, is_rendered: { scope: !state.is_rendered.scope } };

        case ReportActionTypes.MANAGE_EXECUTION_PLAN_TAB_DISPLAY:
            return { ...state, is_rendered: { execution_plan: !state.is_rendered.execution_plan } };

        case ReportActionTypes.MANAGE_RESOURCE_REQUIREMENT_TAB_DISPLAY:
            return { ...state, is_rendered: { resource_requirement: !state.is_rendered.resource_requirement } };

        case ReportActionTypes.MANAGE_BUDGET_TAB_DISPLAY:
            return { ...state, is_rendered: { budget: !state.is_rendered.budget } };

        case ReportActionTypes.MANAGE_SURVEY_INFORMATION_TAB_DISPLAY:
            return { ...state, is_rendered: { survey_information: !state.is_rendered.survey_information } };
